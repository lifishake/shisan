...
<?php else : ?>
    <?php
        ...
        if ( $cookie ) {
            $comment_part.= sprintf('<div class="form_row"><span> %1$s %2$s <i class="show-form" ><span class="glyphicon glyphicon-retweet" title="换马甲"></span></i>， 欢迎回来。</span></div>', $cookie, get_avatar( $email, $size = '24')) ;
            echo $comment_part;
        }
        else {
    ...
    ?>
    <div id="author_info">
    <p class="comment-form-author"><input id="author" name="author" type="text"  size="30" placeholder="昵称（必填，谢绝广告性名词）" value="<?php echo esc_attr($cookie); ?>" maxlength="245" aria-required='true' required='required' /></p>
