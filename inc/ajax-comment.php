 ...
 * 来源: 破袜子原创
 */
function protamine_additional_grasp_show( $email ) {
	?>
 ...
 * 来源: 破袜子原创
 */
function shisan_additional_comment_show( $comment ) {
	?>
        ...
        <article class="comment-body">
            <footer class="comment-meta">
                ...
                <?php
                    ...
                    if ( $comment_meta )
                    {
                        $forbidden = $comment_meta['forbidden'];
                ?>
                        <p class="comment-warnning">因为你的昵称中包含博主不欢迎的词【<?php echo $comment_meta['forbidden']; ?>
                            】，所以昵称、邮箱和网址都已被替换，只保留留言内容。不甘心的话不妨换个昵称挑战一下。</p>
                <?php } ?>
            </footer>
...
function ajax_comment_callback(){
    $dict = 'no-comment-is-best-comment';
    if ( ! wp_verify_nonce($_POST['nonce'],$dict) ) {
        die();
    }
		$comment  = shisan_newcomment( );
		shisan_additional_comment_show($comment );
}
